...
parser = argparse.ArgumentParser(description="Export the efficient-sam decoder to an onnx model.")
parser.add_argument(
    "--checkpoint",
    type=str,
    required=True,
    ...
    if args.simplify:
        try:
            ...
            onnx_model, check = onnxsim.simplify(
                onnx_model,
                10,
                test_input_shapes={name: list(inp.shape) for name, inp in dummy_inputs.items()},
            )
